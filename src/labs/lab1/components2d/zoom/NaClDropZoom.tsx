...
/** Pre: dry slide, drop falling toward it. Post: wet slide with NaCl oval. */
export default function NaClDropZoom({ onDone }: ZoomViewProps) {
  useAutoDone(onDone, 1500);
  const phase = usePhase(650);
  return (
    <div className="absolute inset-0 grid place-items-center">
      <TopDownSlide override={phase === "pre" ? { naclApplied: false } : undefined} />
      {/* Falling drop only in pre-phase */}
      {phase === "pre" && (
        <motion.div
      ...
      {/* Splash ring at the moment of contact */}
      {phase === "post" && (
        <motion.div
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 rounded-full bg-white/85 px-3 py-1 text-xs font-semibold text-slate-800 shadow">
        {phase === "pre" ? "NaCl tomchisi tushyapti" : "NaCl tomchisi qo'yildi"}
      </div>
    </div>
  );
}
