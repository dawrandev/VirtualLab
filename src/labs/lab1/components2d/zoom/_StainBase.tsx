...
/**
 * Generic stain cutscene: large droplets fall onto the slide, color floods
 * outward from the contact point. Re-used by all 4 Gram-stain zooms.
 */
export function StainBase({ onDone, variant, color, label, ms }: Props) {
  useAutoDone(onDone, ms);
  return (
    <div className="absolute inset-0 grid place-items-center">
      <TopDownSlide overrideActiveStain={variant} />
      {/* Falling droplets */}
      {[0, 1, 2].map((i) => (
        <motion.div
          key={i}
          ...
          transition={{ duration: 0.55, delay: i * 0.15, ease: [0.55, 0.05, 0.6, 1] }}
          style={{
            width: 24,
            height: 24,
      ...
      <motion.div
        ...
        transition={{ duration: ms / 1000 - 0.3, delay: 0.4 }}
