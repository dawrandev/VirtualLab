...
const MARK: Record<StepStatus, { icon: string; color: string; bg: string; note: string }> = {
  full: { icon: "✓", color: "#059669", bg: "#ecfdf5", note: "to'liq bajarildi" },
  partial: { icon: "≈", color: "#d97706", bg: "#fffbeb", note: "to'liqsiz" },
  zero: { icon: "✕", color: "#dc2626", bg: "#fef2f2", note: "bajarilmadi" },
};

/** End-of-exam breakdown for Lab 2 — the five PDF criteria graded
 *  full / partial / zero, with reasons, summing to /100. */
export function Lab2ResultModal({ result, onRestart, onViewScope }: Props) {
      ...
      >
        ...
        <div className="flex items-center gap-4 px-6 py-5" style={{ background: `linear-gradient(135deg, ${t.color}22, #ffffff)` }}>
          ...
          <div>
            ...
            <p className="text-[13px] text-slate-500">{tr("lab2.result.subtitle")}</p>
          </div>
        </div>
        ...
        <div className="wb-tray flex-1 space-y-2.5 overflow-y-auto px-5 py-4">
          ...
          <div
            ...
            style={{ background: result.classification.isRight ? "#ecfdf5" : "#fef2f2" }}
          >
            ...
            <div className="text-[13px]">
              <p className="font-semibold" style={{ color: result.classification.isRight ? "#059669" : "#dc2626" }}>
                {result.classification.picked == null
                  ? tr("lab2.result.noMorph")
                  : result.classification.isRight
                    ? tr("lab2.result.right")
                    : tr("lab2.result.wrong")}
              </p>
              <p className="text-slate-500">{tr("lab2.result.correctAnswer", { ans: result.classification.correct === "positive" ? tr("lab2.result.ansPositive") : tr("lab2.result.ansNegative") })}</p>
            </div>
          </div>
            ...
            return (
              <div key={s.id} className="rounded-xl px-3 py-2.5" style={{ background: m.bg }}>
                <div className="flex items-center gap-3">
                  <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full text-sm font-bold" style={{ color: m.color, border: `1.5px solid ${m.color}` }}>
                    {m.icon}
                  </span>
                  <p className="flex-1 text-[13px] font-medium leading-snug text-slate-700">{tr(s.label)}</p>
                  <span className="shrink-0 text-[12px] font-bold tabular-nums" style={{ color: m.color }}>
                    {s.earned} / {s.full}
                  </span>
                </div>
                {s.notes.length > 0 && (
        ...
        <div className="flex gap-2 border-t border-slate-200 p-3">
          <button onClick={onViewScope} className="flex-1 rounded-xl bg-slate-100 px-4 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-200">
            {tr("lab2.result.viewScope")}
          </button>
          <button onClick={onRestart} className="flex-1 rounded-xl bg-emerald-500 px-4 py-2.5 text-sm font-semibold text-white shadow transition hover:bg-emerald-400">
            {tr("ui.restart")}
          </button>
        </div>
      </motion.div>
