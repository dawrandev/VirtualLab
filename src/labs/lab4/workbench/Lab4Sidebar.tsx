...
interface Props {
  state: DiskState;
  placed: Set<Lab4ItemId>;
  draggingId: Lab4ItemId | null;
  carrying: string | null;
  onStartDrag: (id: Lab4ItemId, e: React.PointerEvent) => void;
  showHints?: boolean;
}

/** Left tool tray for Lab 4 — same paradigm as Labs 1–3. */
export function Lab4Sidebar({ state, placed, draggingId, carrying, onStartDrag, showHints = true }: Props) {
  const [hovered, setHovered] = useState<Lab4ItemId | null>(null);
  const t = useTranslations();
  const required = showHints ? requiredItem(state, carrying) : null;
  ...
  return (
    <aside className="relative z-20 flex h-full w-[212px] shrink-0 flex-col border-r border-slate-300/70" style={{ background: "linear-gradient(180deg,#fbfbfc 0%,#eef0f3 100%)" }}>
      ...
      <div className="wb-tray grid flex-1 grid-cols-2 content-start gap-2 overflow-y-auto px-3 py-3">
        {LAB4_ITEMS.filter((item) => !item.fixed && !(item.id === "match" && state.match.discarded)).map((item) => {
          ...
          return (
            <div key={item.id} className="relative">
              <button
                ...
                style={{
                  border: isRequired ? "2px solid #0284c7" : "2px solid #e2e8f0",
                  boxShadow: isRequired ? "0 0 0 1px #38bdf8, 0 0 12px rgba(2,132,199,0.4)" : "0 1px 2px rgba(0,0,0,0.08)",
              ...
              >
                ...
                {isPlaced && <span className="pointer-events-none text-[8px] uppercase tracking-wider text-sky-600">{t("ui.onTable")}</span>}
              </button>
              {hovered === item.id && (
