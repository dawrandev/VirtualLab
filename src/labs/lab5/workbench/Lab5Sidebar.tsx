...
interface Props {
  state: WetMountState;
  placed: Set<Lab5ItemId>;
  draggingId: Lab5ItemId | null;
  onStartDrag: (id: Lab5ItemId, e: React.PointerEvent) => void;
  showHints?: boolean;
}
  ...
  return (
    <aside className="relative z-20 flex h-full w-[212px] shrink-0 flex-col border-r border-slate-300/70" style={{ background: "linear-gradient(180deg,#fbfbfc 0%,#eef0f3 100%)" }}>
      ...
      <div className="wb-tray grid flex-1 grid-cols-2 content-start gap-2 overflow-y-auto px-3 py-3">
        {LAB5_ITEMS.filter((item) => !(item.id === "match" && state.match.discarded) && !(item.id === "coverslip" && state.coverPlaced)).map((item) => {
            ...
            <div key={item.id} className="relative">
              <button
                ...
                style={{
                  ...
                  boxShadow: isRequired ? "0 0 0 1px #fbbf24, 0 0 12px rgba(217,119,6,0.4)" : "0 1px 2px rgba(0,0,0,0.08)",
